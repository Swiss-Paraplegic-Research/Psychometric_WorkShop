PI_map = function(object, pp){
     # object = Thr_PCM 
     # pp = round(person_parameters,4)
   # 
 warn.ord.colour = "red"
 cex.gen = 1.
...
for (j in 1:nrow(tr)) {
 #  j = 26
   ...
   # points(loc[i], j, pch = 20, cex = 1, col = "white")
}
  ...
  # saveRDS(list(Dens_Seg = tr.rug,
  # IP = tr,
  # PP = cbind(ttx, tt)), paste0(path_output_spec, "PIMAP_values.rds"))
